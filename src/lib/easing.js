...
export const getEasingValues = (easing) =>
  eases[easing].match(/(\d+\.?\d*)/g).map(parseFloat);

// export const getFramerMotionEasing = (easing) => {
//   return cubicBezier(...getEasingValues(easing));
// };
...
export function inOutElastic(n) {
  ...
  if ((n *= 2) < 1) {
    return (
      -0.5 *
      (a * 2 ** (10 * (n -= 1)) * Math.sin(((n - s) * (2 * Math.PI)) / p))
    );
  }
  return (
    a * 2 ** (-10 * (n -= 1)) * Math.sin(((n - s) * (2 * Math.PI)) / p) * 0.5 +
    1
  );
}

// export const EASE_CUSTOM_1 = CustomEase.create(
//   "customEase1",
//   ".25, .46, .45, .94"
// );
// export const EASE_CUSTOM_2 = CustomEase.create("customEase2", ".19, 1, .22, 1");
// export const EASE_CUSTOM_3 = CustomEase.create(
//   "customEase3",
//   ".77, 0, .175, 1"
// );
// export const EASE_CUSTOM_4 = CustomEase.create("customEase4", ".22, 1, .36, 1");

// export const EASE_CUSTOM_5 = CustomEase.create(
//   "customEase5",
//   "0.215, 1.61, 0.355, 1"
// );
