...
class GlyphsGeometry extends InstancedBufferGeometry {
  constructor() {
    ...
    // Start with zero instances until updateGlyphs is called
    this.instanceCount = 0

    // Define groups for rendering text outline as a separate pass; these will only
    // be used when the `material` getter returns an array, i.e. outlineWidth > 0.
    // Note: Using actual count instead of Infinity for WebGPU compatibility
    this.groups = [
    ...
    // Initialize with dummy instanced attributes so NodeMaterial can compile
    // These will be replaced when updateGlyphs is called
  ...
  updateGlyphs(glyphBounds, glyphAtlasIndices, blockBounds, chunkedBounds, glyphColors) {
    ...
    // Update groups to use actual count
    this.groups[0].count = this.instanceCount
    this.groups[1].count = this.instanceCount
