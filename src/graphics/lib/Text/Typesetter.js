...
export function createTypesetter(resolveFonts, bidi) {
  ...
  function calculateFontRuns(
    ...
    console.log("[calculateFontRuns] Starting with fonts:", fonts);
    const onResolved = ({ chars, fonts: parsedFonts }) => {
      console.log("[calculateFontRuns] Fonts resolved:", parsedFonts);
      let curRun, prevVal;
      const runs = [];
    ...
    if (preResolvedFonts) {
      console.log("[calculateFontRuns] Using pre-resolved fonts");
      onResolved(preResolvedFonts);
    } else {
      console.log("[calculateFontRuns] Calling resolveFonts");
  ...
  function typeset(
    ...
    console.log("[Typesetter] Starting typeset for text:", text);
    const mainStart = now();
    ...
    calculateFontRuns(
      ...
      (runs) => {
        console.log("[Typesetter] Font runs calculated:", runs);
        ...
        lines.forEach((line) => {
          ...
          for (let i = line.count; i--; ) {
            ...
            let { lineHeight, capHeight, xHeight, baseline } =
              glyphInfo.fontData;
            ...
            if (lineGlyphCount > 0) {
              ...
              for (let i = 0; i < lineGlyphCount; i++) {
                ...
                if (includeCaretPositions) {
                  ...
                  caretPositions[charIndex * 4 + 1] = rtl
                    ? caretLeft
                    : caretRight; //end edge x
                  caretPositions[charIndex * 4 + 2] =
                    line.baseline + fontData.caretBottom + anchorYOffset; //common bottom y
                  caretPositions[charIndex * 4 + 3] =
                    line.baseline + fontData.caretTop + anchorYOffset; //common top y
