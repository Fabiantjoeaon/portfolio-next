        ...
        for (let i = startIndex, iLen = text.length; i < iLen; i++) {
          ...
          if (
            (prevCharResult === RESOLVED &&
              fontResolutions[charResolutions[i - 1]].supportsCodePoint(
                codePoint
              )) ||
            (i > 0 && /\s/.test(text[i]))
          ) {
            ...
            for (
              let j = charResolutions[i], jLen = userFonts.length;
              j <= jLen;
              j++
            ) {
              if (j === jLen) {
          ...
          .then(({ fontUrls, chars }) => {
            ...
            fallbackRanges.forEach((range) => {
              for (let i = 0, endIdx = range[1] - range[0]; i <= endIdx; i++) {
                charResolutions[range[0] + i] =
                  chars[charIdx++] + fontIndexOffset;
              }
